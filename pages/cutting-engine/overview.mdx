...
This row cannot be deleted by `manager` or below. Corrections require a `super_admin` action, which itself writes to `audit_log` with `before_value` and `after_value` JSONB snapshots.
